...
describe('BillingPage', () => {
  ...
  test('shows validation error for invalid amount', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(/Enter a valid billing amount/i);
  });
  ...
  test('submits and shows success when API returns invoice', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('status')).toHaveTextContent(/Invoice created/));
  });
  ...
  test('shows validation error when amount exceeds maximum', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(/must not exceed/i);
  });

  test('shows error when billing API throws', async () => {
    vi.spyOn(billingApiModule.billingApi, 'createInvoice').mockRejectedValue(new Error('Billing failed'));
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(/billing failed/i);
  });

  test('shows empty state when API returns no invoice', async () => {
    vi.spyOn(billingApiModule.billingApi, 'createInvoice').mockResolvedValue(null);
    ...
    expect(await screen.findByRole('status')).toHaveTextContent(/No invoice generated for this tenant/i);
  });
  ...
  test('resets to form state after success and allows another invoice', async () => {
    ...
    fireEvent.click(screen.getByRole('button', {name: /create another invoice/i}));
