...
import {SUPPORTED_BRANDS, tokenSetForBrand} from './themeRegistry';
...
export function ThemeBoot({children}: {children: React.ReactNode}) {
  ...
  const brandId = ensureSupported(params.get('brand'), SUPPORTED_BRANDS, DEFAULT_TENANT.brandId);
  const locale = ensureSupported(params.get('locale'), SUPPORTED_LOCALES, DEFAULT_TENANT.locale);
  ...
  const tokenSet = tokenSetForBrand(tenant.brandId);
