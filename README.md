...
**Built for GitHub Pages.** Pages has no routing, so the build output is shaped for it. Pages would redirect `/livery/onyx/invoices` to a trailing slash if the file were `onyx/invoices/index.html`, so each page is written as `onyx/invoices.html`. For any other address Pages returns `404.html` with status 404; that file is the client-rendered shell React Router builds for URLs it did not prerender, and it shows a not-found page in the default look. An unknown page of a known tenant reaches the same page through a catch-all route.
...
| Unit       | Vitest, Testing Library | the payment machine, card checks, the mock API, and the home, sign-in and invoice pages against it, payment flows included - 30 tests                                                                                                            |
| End-to-end | Playwright              | the production build on desktop and a Pixel 7: the account and invoices on all three brands, card payments, declines, bank confirmation, flags, sign-in redirects, prerendered tokens, pages without JavaScript, 404s, components - 26 scenarios |
