...
describe('parseTenantConfig', () => {
  it('reads a valid tenant, whose tokens default to its own folder', () => {
    expect(parse({name: 'Acme', locale: 'en-GB', currency: 'GBP'}).config).toEqual({
      id: 'acme',
      name: 'Acme',
      tokens: 'acme',
      locale: 'en-GB',
  ...
  it("lets a tenant borrow another tenant's look", () => {
    expect(
      parse({name: 'Acme Lite', tokens: 'house', locale: 'en-US', currency: 'USD'}, 'acme-lite').config?.tokens
    ).toBe('house');
  });

  it('reports every problem', () => {
    const {config, messages} = parse(
      {name: ' ', tokens: 'nobody', locale: 'en_GB', currency: 'pounds', colour: 'red'},
      'Acme_1'
    );
    expect(config).toBeUndefined();
    expect(messages).toEqual([
      '(folder): "Acme_1" cannot be a tenant id: use lower-case letters, digits and single hyphens',
      'colour: is not a tenant setting; use name, tokens, locale and currency',
  ...
  it('wants canonical locales, as they are also used in URLs and HTML', () => {
    expect(parse({name: 'Acme', locale: 'en-gb', currency: 'GBP'}).messages).toEqual([
      'locale: must be a canonical BCP 47 locale, such as "en-GB"',
    ]);
  });
});
